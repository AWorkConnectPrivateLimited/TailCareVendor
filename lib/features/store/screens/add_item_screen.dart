...
class _AddItemScreenState extends State<AddItemScreen> {
  ...
  void initState() {
    ...
    final List<Translation> translations = args?['translations'] ?? [];
    final Map<String, dynamic>? prefillData = args?['prefillData']; // Extract prefillData

    print("itemReceived PrefillData in AddItemScreen: $prefillData");

    print('Prefill category Data: ${prefillData?['category']?.toString()}');
    print('Prefill category Data: ${prefillData?['subcategory']?.toString()}');

    _maxOrderQuantityController.text = prefillData?['maxOrderQuantity']?.toString() ?? '';

    _priceController.text = prefillData?['price']?.toString() ?? '';
    _discountController.text = prefillData?['discount']?.toString() ?? '';
    _stockController.text = prefillData?['totalStock']?.toString() ?? '';



      cat = prefillData?['category']?.toString() ?? '';
      uni = prefillData?['unit']?.toString() ?? '';
      scat = prefillData?['subcategory']?.toString() ?? '';
      img1 = prefillData?['imageUrl1']?.toString() ?? '';
      img2 = prefillData?['imageUrl2']?.toString() ?? '';
      img3 = prefillData?['imageUrl3']?.toString() ?? '';
      img4 = prefillData?['imageUrl4']?.toString() ?? '';
      img5 = prefillData?['imageUrl5']?.toString() ?? '';
      img6 = prefillData?['imageUrl6']?.toString() ?? '';
      img7 = prefillData?['imageUrl7']?.toString() ?? '';
      img8 = prefillData?['imageUrl8']?.toString() ?? '';
      img9 = prefillData?['imageUrl9']?.toString() ?? '';
      img10 = prefillData?['imageUrl10']?.toString() ?? '';
      img1 = prefillData?['imageUrl1']?.toString() ?? '';
              ...
              Expanded(child: SingleChildScrollView(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Row(children: [
                    ...
                    Expanded(
                      child: CustomDropDownWidget(
                        value: (cat != null && categoryList.contains(cat))
                            ? (categoryList.indexOf(cat) + 1).toString()  // Convert to String as the value parameter expects a String
                            : categoryController.categoryIndex.toString(), // Fallback to the controller's current index if cat is null
                        title: 'category'.tr,
                        dataList: categoryList,
                        onChanged: (String value) {
                          // Set the selected category index in the controller
                          categoryController.setCategoryIndex(int.parse(value), true);

                          // If the selected category is not "0" (i.e. valid), fetch subcategory list
                          if (value != '0') {
                            categoryController.getSubCategoryList(
                              categoryController.categoryList![int.parse(value) - 1].id,
                              null,
                            );
                          }
                        },
                    ...
                    Expanded(
                      child: CustomDropDownWidget(
                        value: (subCategory != null && subCategory.contains(scat))
                            ? (subCategory.indexOf(scat) + 1).toString()
                            : categoryController.subCategoryIndex.toString(), // Fallback to controller's current index
                        title: 'sub_category'.tr,
                        dataList: subCategory,
                        onChanged: (String value) {
                          categoryController.setSubCategoryIndex(int.parse(value), true);
                        },
                      ),
                    ),
                  ...
                  (_module.stock! || _module.unit!) ? Row(children: [
                    ...
                    Expanded(
                      child: CustomDropDownWidget(
                        // Check if there is a prefilledUnitValue and if it exists in the unitList
                        value: (uni != null && unitList.contains(uni))
                            ? (unitList.indexOf(uni) + 1).toString()
                            : storeController.unitIndex.toString(),
                        title: 'unit'.tr,
                        dataList: unitList,
                        onChanged: (String value) {
                          storeController.setUnitIndex(int.parse(value), true);
                        },
              ...
              !storeController.isLoading ? CustomButtonWidget(
                ...
                onPressed: () {
                  ...
                  if(price.isEmpty) {
                    ...
                    _item.brandId = storeController.brandList != null && storeController.brandList!.isNotEmpty ? storeController.brandList![storeController.brandIndex! - 1].id : 0;
