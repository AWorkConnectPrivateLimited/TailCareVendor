...
class AddNameScreen extends StatefulWidget {
  ...
  const AddNameScreen({Key? key, this.item, this.prefillData}) : super(key: key);
...
class AddNameScreenState extends State<AddNameScreen> {
  ...
  int numLanguages = 3; // Replace with your actual number of languages
  //late List<TextEditingController> _nameControllerList;
  ...
  void initState() {
    super.initState();
    print("Received PrefillData: ${widget.prefillData}");

    int numLanguages = 2;

    for (int i = 0; i < numLanguages; i++) {
      _nameControllerList.add(TextEditingController());
      _descriptionControllerList.add(TextEditingController()); // Add description controllers
    }

    if (widget.prefillData != null) {
      for (int i = 0; i < _nameControllerList.length; i++) {
        if(i == 0) {
          _nameControllerList[i].text = widget.prefillData!['name'] ?? ''; // English name
          _descriptionControllerList[i].text = widget.prefillData!['description'] ?? ''; // English description
        } else {
          // Handle other languages (leave empty or prefill differently)
          _nameControllerList[i].text = '';
          _descriptionControllerList[i].text = '';
        }
      }
            ...
            CustomButtonWidget(
              ...
              onPressed: () {
                ...
                if(defaultDataNull) {
                  ...
                  Get.toNamed(
                    RouteHelper.getAddItemRoute(widget.item, translations),
                    arguments: {
                      'translations': translations,
                      'prefillData': widget.prefillData, // Pass prefillData here
                    },
                  );
                }
              },
